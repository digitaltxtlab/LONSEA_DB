...
mdl_df <- direc %>% 
  ...
  filter(!is.na(direc$begin_year) & !is.na(direc$end_year))
...
seven_nat_direc %>% 
  ...
  geom_bar(width = 1)+
  scale_x_continuous(breaks = seq(0,30, by = 2))+
  labs(x = "Length in years", y = "Number of directors", title = "European Directors: Distribution of director periods")
